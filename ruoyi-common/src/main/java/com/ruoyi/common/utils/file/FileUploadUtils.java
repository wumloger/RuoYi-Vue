...
public class FileUploadUtils
{
    ...
    public static final String uploadMinioWithDir(MultipartFile file, String baseDir) throws IOException {
        try {
            String bucketName = getBucketName();
            String objectName = baseDir + "/" + file.getOriginalFilename();
            return uploadMinino(bucketName, file, MimeTypeUtils.DEFAULT_ALLOWED_EXTENSION,objectName);
        } catch (Exception e) {
            throw new IOException(e.getMessage(), e);
        }
    }
    ...
    private static final String uploadMinino(String bucketName, MultipartFile file, String[] allowedExtension, String baseDir)
            throws FileSizeLimitExceededException, IOException, FileNameLengthLimitExceededException,
            InvalidExtensionException {
        int fileNamelength = file.getOriginalFilename().length();
        if (fileNamelength > FileUploadUtils.DEFAULT_FILE_NAME_LENGTH) {
        ...
        try {
            ...
            String pathFileName = MinioUtil.uploadFile(bucketName, objectName, file);
            return pathFileName;
        } catch (Exception e) {
            throw new IOException(e.getMessage(), e);
        }
